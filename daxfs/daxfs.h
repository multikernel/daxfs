...
struct daxfs_delta_inode_entry {
	...
	u32 uid;			/* Current uid */
	u32 gid;			/* Current gid */
	u32 base_nlink;			/* nlink from base image (0 if delta-created) */
	s32 nlink_delta;		/* Accumulated change from unlinks */
	bool deleted;			/* True when base_nlink + nlink_delta <= 0 */
...
extern int daxfs_get_effective_nlink(struct super_block *sb, u64 ino, u32 *nlink);
